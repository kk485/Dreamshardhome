...
// ✅ 第 1 步：删除旧图标库，引入 Iconify 和 Vue 的 h/defineComponent 函数
...
// ✅ 第 2 步：因为 el-switch 的属性需要传入“组件对象”，我们用 defineComponent 把 Iconify 的 Icon 包装成标准 Vue 组件
const CheckSmall = defineComponent({
  setup() {
    return () => h(Icon, { icon: "icon-park-outline:check-small" });
  }
});
const CloseSmall = defineComponent({
  setup() {
    return () => h(Icon, { icon: "icon-park-outline:close-small" });
  }
});
...
const radioChange = () => {
  ElMessage({
    message: "壁纸更换成功",
    // ✅ 第 3 步：替换 h() 函数中的 SuccessPicture，直接渲染 Icon 并设置样式颜色
    icon: h(Icon, { 
      icon: "icon-park-solid:success-picture", 
      style: { color: "var(--el-message-icon-color)" } 
    }),
  });
  if (store.webSpeech) {
    stopSpeech();
